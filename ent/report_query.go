...
func (rq *ReportQuery) QueryScript() *ScriptQuery {
	query := (&ScriptClient{config: rq.config}).Query()
...
func (rq *ReportQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (rq *ReportQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (rq *ReportQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (rq *ReportQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (rq *ReportQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (rq *ReportQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := rq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (rq *ReportQuery) Clone() *ReportQuery {
	...
	return &ReportQuery{
		...
		withScript: rq.withScript.Clone(),
...
func (rq *ReportQuery) WithScript(opts ...func(*ScriptQuery)) *ReportQuery {
	query := (&ScriptClient{config: rq.config}).Query()
...
func (rq *ReportQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Report, error) {
	...
	if query := rq.withScript; query != nil {
		if err := rq.loadScript(ctx, query, nodes, nil,
			func(n *Report, e *Script) { n.Edges.Script = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReportQuery) loadScript(ctx context.Context, query *ScriptQuery, nodes []*Report, init func(*Report), assign func(*Report, *Script)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Report)
	for i := range nodes {
		if nodes[i].script_reports == nil {
			continue
		}
		fk := *nodes[i].script_reports
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "script_reports" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *ReportQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(report.Table, report.Columns, sqlgraph.NewFieldSpec(report.FieldID, field.TypeUUID))
	_spec.From = rq.sql
