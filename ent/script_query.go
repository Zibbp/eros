...
func (sq *ScriptQuery) QueryReports() *ReportQuery {
	query := (&ReportClient{config: sq.config}).Query()
...
func (sq *ScriptQuery) Clone() *ScriptQuery {
	...
	return &ScriptQuery{
		...
		withReports: sq.withReports.Clone(),
...
func (sq *ScriptQuery) WithReports(opts ...func(*ReportQuery)) *ScriptQuery {
	query := (&ReportClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withReports = query
	return sq
}
...
func (sq *ScriptQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Script, error) {
	...
	if query := sq.withReports; query != nil {
		if err := sq.loadReports(ctx, query, nodes,
			func(n *Script) { n.Edges.Reports = []*Report{} },
			func(n *Script, e *Report) { n.Edges.Reports = append(n.Edges.Reports, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ScriptQuery) loadReports(ctx context.Context, query *ReportQuery, nodes []*Script, init func(*Script), assign func(*Script, *Report)) error {
	...
	query.Where(predicate.Report(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(script.ReportsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.script_reports
		if fk == nil {
			return fmt.Errorf(`foreign-key "script_reports" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "script_reports" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
